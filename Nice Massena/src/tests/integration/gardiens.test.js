...
describe('Gardiens API', () => {
  ...
  describe('GET /api/gardiens', () => {
    test('should return empty array when no gardiens exist', async () => {
      ...
      expect(response.body).toEqual({ result: true, gardiens: [] });
    });
    ...
    test('should return all gardiens', async () => {
      ...
      expect(response.body.gardiens).toHaveLength(2);
      expect(response.body.gardiens[0].name).toBe('John Hammond');
      expect(response.body.gardiens[1].name).toBe('Sarah Connor');
    });
  });
  ...
  describe('GET /api/gardiens/:id', () => {
    test('should return gardien by id', async () => {
      ...
      expect(response.body.result).toBe(true);
      expect(response.body.gardien.name).toBe('John Hammond');
      expect(response.body.gardien.specialty).toBe('carnivores');
    });
    ...
    test('should return 404 for non-existent gardien', async () => {
      ...
      expect(response.body.error).toBe('Gardien non trouvé');
    });
  ...
  describe('POST /api/gardiens', () => {
    test('should create new gardien', async () => {
      ...
      expect(response.body.result).toBe(true);
      expect(response.body.gardien.name).toBe('Alan Grant');
      expect(response.body.gardien._id).toBeDefined();

      // Verify it's in database
      const savedGardien = await Gardiens.findById(response.body.gardien._id);
  ...
  describe('PUT /api/gardiens/:id', () => {
    test('should update existing gardien', async () => {
      ...
      expect(response.body.result).toBe(true);
      expect(response.body.gardien.name).toBe('John Hammond Updated');
      expect(response.body.gardien.specialty).toBe('medical');
      expect(response.body.gardien.available).toBe(false);
    });
  ...
  describe('DELETE /api/gardiens/:id', () => {
    test('should delete existing gardien', async () => {
      ...
      expect(response.body.result).toBe(true);
      expect(response.body.message).toBe('Gardien supprimé !');
